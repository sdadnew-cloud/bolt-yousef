 ...
 */
function getEncoding(buffer: Buffer, opts?: { chunkLength?: number; chunkBegin?: number }): 'utf8' | 'binary' | null {
  // Check
  if (!buffer)
    return null;
...
function getChunkBegin(buf: Buffer, chunkBegin: number): number {
...
function getChunkEnd(buf: Buffer, chunkEnd: number): number {
...
function isFirstByteOf4ByteChar(byte: number): boolean {
  return (byte >> 3) === 30; // 11110xxx?
}

function isFirstByteOf3ByteChar(byte: number): boolean {
  return (byte >> 4) === 14; // 1110xxxx?
}

function isFirstByteOf2ByteChar(byte: number): boolean {
  return (byte >> 5) === 6; // 110xxxxx?
}

function isLaterByteOfUtf8(byte: number): boolean {
